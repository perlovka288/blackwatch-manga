...
/* ===== TOP WEEK (ГОРЯЧЕЕ) ===== */
.top-week-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(185px,1fr));gap:16px;margin-top:14px}
.top-week-card{background:var(--card);border:1px solid var(--border);border-radius:12px;overflow:hidden;text-decoration:none;color:var(--text);transition:all 0.3s;display:flex;flex-direction:column}
.top-week-card:hover{transform:translateY(-6px);border-color:var(--accent);box-shadow:0 12px 32px rgba(0,0,0,0.5)}
.top-week-cover{width:100%;aspect-ratio:2/3;background:var(--card2);background-size:cover;background-position:center;border-bottom:1px solid var(--border)}
.top-week-cover-ph{width:100%;aspect-ratio:2/3;display:flex;align-items:center;justify-content:center;background:var(--card2);font-size:48px}
.top-week-info{padding:12px;flex:1;display:flex;flex-direction:column}
.top-week-title{font-size:12px;font-weight:600;color:var(--text);margin-bottom:8px;overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;line-height:1.35}
.top-week-stats{font-size:10px;color:var(--muted);margin-bottom:6px;display:flex;gap:8px}
.top-week-rating{display:flex;align-items:center;gap:3px;padding:6px 8px;background:rgba(232,25,44,0.15);border-radius:6px;font-size:10px;font-weight:600;color:var(--accent);justify-content:center;margin-bottom:8px}
.top-week-btn{width:100%;padding:8px 12px;background:var(--accent);color:#fff;border:none;border-radius:6px;font-weight:600;font-size:11px;cursor:pointer;transition:all 0.2s;margin-top:auto}
.top-week-btn:hover{background:var(--accent2);transform:translateY(-1px)}
...
try {
    ...
    $weeklyTop = $topWeekStmt->fetchAll();
} catch (Exception $e) {
    $weeklyTop = [];
}
?>
...
const TW_CARD_W = 292; // ширина карточки + gap
const TOP_WEEK_VISIBLE = 4;
            ...
            track.innerHTML = data.items.map((m, i) => {
                ...
                <div style="flex:0 0 280px;background:var(--card);border:1px solid var(--border);border-radius:12px;padding:10px;display:flex;gap:11px;align-items:flex-start;transition:border-color .2s,box-shadow .2s" onmouseover="this.style.borderColor='var(--accent)';this.style.boxShadow='0 8px 28px rgba(0,0,0,0.5)'" onmouseout="this.style.borderColor='var(--border)';this.style.boxShadow=''">
                    ...
                    <div style="flex:1;min-width:0;display:flex;flex-direction:column;gap:5px;padding-top:2px">
                        <div style="font-size:12px;font-weight:700;color:var(--text);line-height:1.35;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden">${escapeHtml(m.title)}</div>
                        <div style="font-size:10px;color:var(--muted)">Язык - Русский</div>
                        ${ratingVal ? `<div style="font-size:10px;color:#f59e0b;font-weight:600">★ ${ratingVal} / 10</div>` : `<div style="font-size:10px;color:var(--muted)">♥ ${m.likes||0}</div>`}
                        <a href="/read/${m.id}" style="display:block;background:var(--accent);color:#fff;font-size:11px;font-weight:700;text-align:center;padding:7px 10px;border-radius:7px;text-decoration:none;transition:background .18s;margin-top:auto" onmouseover="this.style.background='var(--accent2)'" onmouseout="this.style.background='var(--accent)'">Читать</a>
                    </div>
                </div>`;
            }).join('');
            topWeekOffset = 0;
...
function topWeekSlide(dir) {
    ...
    const track = document.getElementById('top-week-track');
    if (track) track.style.transform = `translateX(-${topWeekOffset * TW_CARD_W}px)`;
    updateTopWeekArrows();
}
