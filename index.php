...
if (preg_match('#^/api/pages/(\\d+)$#', $path, $m)) {
    ...
    if (empty($pages)) {
        ...
        $s->execute([$id]);
        echo json_encode(['pages' => [], 'telegraph' => $s->fetchColumn() ?? null]);
    } else {
...
<script>
    ...
    async function load(reset = false) {
        ...
        hasMore = (page + 1) * d.limit < d.total; 
