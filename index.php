...
# ========================= MANGADEX API =========================
// Route MangaDex API calls through mangadex.php
if (str_starts_with($path, '/api/mdex/')) {
    require __DIR__ . '/mangadex.php';
    exit;
}
...
<div class="modal admin-modal">
    ...
    <div class="admin-tabs">
        ...
        <button class="atab" onclick="switchAdminTab('mdex')">🌐 Добавить с сервисов (тест)</button>
    </div>
    ...
    <!-- MANGADEX PANEL (ТЕСТ) -->
    <div class="apanel" id="panel-mdex">
        <div style="background:rgba(255,107,53,0.08);border:1px solid rgba(255,107,53,0.25);border-radius:12px;padding:10px 14px;margin-bottom:16px;font-size:12px;color:#ffb347">
            ⚠️ <strong>ТЕСТ</strong> — Импорт глав напрямую с MangaDex. Страницы скачиваются на сервере и загружаются на ImgBB. Может занять несколько минут.
        </div>

        <div style="margin-bottom:14px">
            <div style="font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:8px">Язык перевода</div>
            <div style="display:flex;gap:6px;flex-wrap:wrap">
                <button class="mdex-lang-btn active" id="mdex-lang-ru" onclick="setMdexLang('ru')">🇷🇺 Русский</button>
                <button class="mdex-lang-btn" id="mdex-lang-en" onclick="setMdexLang('en')">🇬🇧 English</button>
                <button class="mdex-lang-btn" id="mdex-lang-all" onclick="setMdexLang('')">🌐 Все языки</button>
            </div>
        </div>

        <div class="fg">
            <label class="fl">Поиск манги на MangaDex</label>
            <div class="esearch-row">
                <input class="esearch-inp" id="mdex-search" type="text" placeholder="Название (рус / eng)...">
                <button class="esearch-btn" onclick="mdexSearch()">Найти</button>
            </div>
        </div>
        <div id="mdex-results" style="max-height:280px;overflow-y:auto;display:flex;flex-direction:column;gap:7px;margin-bottom:14px"></div>

        <div id="mdex-selected-wrap" style="display:none">
            <div style="font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:8px">Выбранная манга</div>
            <div id="mdex-selected-card" style="background:rgba(124,92,255,0.08);border:1px solid rgba(124,92,255,0.25);border-radius:10px;padding:10px 13px;display:flex;gap:10px;align-items:center;margin-bottom:12px"></div>

            <div style="font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:8px">Куда добавить</div>
            <div style="display:flex;gap:6px;margin-bottom:12px;flex-wrap:wrap">
                <button class="mdex-action-btn active" id="mdex-act-new" onclick="setMdexAction('new_manga')">➕ Новая манга</button>
                <button class="mdex-action-btn" id="mdex-act-exist" onclick="setMdexAction('add_to_existing')">📚 К существующей</button>
            </div>

            <div id="mdex-exist-wrap" style="display:none;margin-bottom:12px">
                <div class="esearch-row">
                    <input class="esearch-inp" id="mdex-exist-search" type="text" placeholder="Найти мангу в вашей БД...">
                    <button class="esearch-btn" onclick="mdexSearchExisting()">Найти</button>
                </div>
                <div id="mdex-exist-results" style="max-height:160px;overflow-y:auto;display:flex;flex-direction:column;gap:5px;margin-top:8px"></div>
            </div>

            <div style="font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:8px">Главы</div>
            <div id="mdex-chapters-wrap" style="max-height:250px;overflow-y:auto;display:flex;flex-direction:column;gap:5px;margin-bottom:12px">
                <div style="color:var(--muted);font-size:13px;padding:8px 0">Нажми "Загрузить список глав" ↓</div>
            </div>
            <button class="sbtn" style="margin-bottom:12px;background:rgba(124,92,255,0.15);border:1px solid rgba(124,92,255,0.35);color:var(--accent)" onclick="mdexLoadChapters()">
                <span class="btn-text">📋 Загрузить список глав</span><div class="spinner"></div>
            </button>

            <div id="mdex-import-wrap" style="display:none">
                <div style="font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:8px">Выбранные главы</div>
                <div id="mdex-selected-chapters" style="margin-bottom:10px;font-size:13px;color:var(--accent)">—</div>
                <div class="upbar" id="mdex-progress"><div class="upbar-fill" id="mdex-progress-fill"></div></div>
                <button class="sbtn" id="mdex-import-btn" onclick="mdexImportSelected()">
                    <span class="btn-text">🚀 Импортировать выбранные главы</span><div class="spinner"></div>
                </button>
                <div class="result-banner" id="mdex-result"></div>
            </div>
        </div>
    </div>
...
// ===== MANGADEX =====
let mdexLang='ru', mdexAction='new_manga', mdexSelectedManga=null, mdexSelectedChapters=new Set(), mdexExistingMangaId=null, mdexAllChapters=[];

function setMdexLang(lang){
    mdexLang=lang;
    ['ru','en','all'].forEach(l=>{const btn=document.getElementById('mdex-lang-'+l);if(btn)btn.classList.toggle('active',l===lang||(lang===''&&l==='all'));});
    if(mdexSelectedManga)mdexLoadChapters();
}

function setMdexAction(action){
    mdexAction=action;
    document.getElementById('mdex-act-new').classList.toggle('active',action==='new_manga');
    document.getElementById('mdex-act-exist').classList.toggle('active',action==='add_to_existing');
    document.getElementById('mdex-exist-wrap').style.display=action==='add_to_existing'?'block':'none';
}

async function mdexSearch(){
    const q=document.getElementById('mdex-search').value.trim();
    if(!q){showToast('❌ Введи название!');return;}
    const res_el=document.getElementById('mdex-results');
    res_el.innerHTML='<div style="color:var(--muted);font-size:13px;padding:8px 0">🔍 Поиск...</div>';
    try{
        const lang=mdexLang||'';
        const url='/api/mdex/search?q='+encodeURIComponent(q)+(lang?'&lang='+lang:'')+'&tg_user_id='+getTgUser();
        const res=await fetch(url);const data=await res.json();
        if(!data.results?.length){res_el.innerHTML='<div style="color:var(--muted);font-size:13px;padding:8px 0">😔 Ничего не найдено</div>';return;}
        res_el.innerHTML=data.results.map(m=>`
            <div class="mdex-manga-item" onclick="selectMdexManga('${m.id}','${escapeHtml(m.title).replace(/'/g,"\\'")}','${m.cover_url?escapeHtml(m.cover_url):''}','${escapeHtml(m.status||'')}')">
                ${m.cover_url?`<img class="mdex-manga-cover" src="${escapeHtml(m.cover_url)}" onerror="this.style.display='none'">`:'<div class="mdex-manga-cover" style="display:flex;align-items:center;justify-content:center">📖</div>'}
                <div style="flex:1;min-width:0">
                    <div class="mdex-manga-title">${escapeHtml(m.title)}</div>
                    <div class="mdex-manga-meta">${m.year?m.year+' • ':''}${m.status||''}</div>
                </div>
            </div>`).join('');
    }catch(e){res_el.innerHTML='<div style="color:#ff5050;font-size:13px">❌ Ошибка: '+escapeHtml(e.message)+'</div>';}
}
document.getElementById('mdex-search').addEventListener('keydown',e=>{if(e.key==='Enter')mdexSearch();});

function selectMdexManga(id,title,coverUrl,status){
    mdexSelectedManga={id,title,coverUrl,status};
    mdexSelectedChapters=new Set();
    mdexAllChapters=[];
    document.getElementById('mdex-results').innerHTML='';
    document.getElementById('mdex-search').value='';
    document.getElementById('mdex-selected-wrap').style.display='block';
    document.getElementById('mdex-import-wrap').style.display='none';
    document.getElementById('mdex-chapters-wrap').innerHTML='<div style="color:var(--muted);font-size:13px;padding:8px 0">Нажми "Загрузить список глав" ↓</div>';
    document.getElementById('mdex-selected-card').innerHTML=`
        ${coverUrl?`<img src="${escapeHtml(coverUrl)}" style="width:40px;height:56px;object-fit:cover;border-radius:7px;flex-shrink:0" onerror="this.style.display='none'">`:'<span style="font-size:28px">📖</span>'}
        <div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:700;color:var(--accent)">${escapeHtml(title)}</div><div style="font-size:11px;color:var(--muted);margin-top:2px">MangaDex ID: ${escapeHtml(id)}</div></div>
        <button onclick="deselectMdexManga()" style="background:none;border:none;color:var(--muted);cursor:pointer;font-size:18px;padding:4px">✕</button>`;
    showToast('✅ Выбрано: '+title);
}

function deselectMdexManga(){
    mdexSelectedManga=null;
    document.getElementById('mdex-selected-wrap').style.display='none';
}

async function mdexLoadChapters(){
    if(!mdexSelectedManga){showToast('❌ Сначала выбери мангу!');return;}
    const wrap=document.getElementById('mdex-chapters-wrap');
    wrap.innerHTML='<div style="color:var(--muted);font-size:13px;padding:8px 0">⏳ Загрузка глав...</div>';
    document.getElementById('mdex-import-wrap').style.display='none';
    mdexSelectedChapters=new Set();
    try{
        const lang=mdexLang||'';
        const url='/api/mdex/chapters?manga_id='+encodeURIComponent(mdexSelectedManga.id)+(lang?'&lang='+lang:'')+'&tg_user_id='+getTgUser();
        const res=await fetch(url);const data=await res.json();
        if(!data.chapters?.length){wrap.innerHTML='<div style="color:var(--muted);font-size:13px;padding:8px 0">😔 Глав не найдено для выбранного языка</div>';return;}
        mdexAllChapters=data.chapters;
        wrap.innerHTML=`
            <div class="mdex-select-all-row">
                <input type="checkbox" id="mdex-check-all" onchange="mdexToggleAll(this.checked)">
                <label for="mdex-check-all">Выбрать все (${data.chapters.length})</label>
                <button onclick="mdexImportWrap()" style="margin-left:auto;padding:4px 12px;background:rgba(124,92,255,0.15);border:1px solid rgba(124,92,255,0.35);border-radius:8px;color:var(--accent);font-size:11px;font-weight:700;cursor:pointer;font-family:inherit">✅ К импорту</button>
            </div>
            ${data.chapters.map(ch=>`
            <div class="mdex-chapter-item" id="mdex-ch-${escapeHtml(ch.id)}" onclick="mdexToggleChapter('${escapeHtml(ch.id)}','${ch.chapter}')">
                <input type="checkbox" id="mdex-cb-${escapeHtml(ch.id)}" onclick="event.stopPropagation();mdexToggleChapter('${escapeHtml(ch.id)}','${ch.chapter}')">
                <span class="mdex-ch-num">Гл. ${escapeHtml(String(ch.chapter||'?'))}</span>
                <span class="mdex-ch-title">${ch.title?escapeHtml(ch.title):''}</span>
                ${ch.pages>0?`<span class="mdex-ch-pages">${ch.pages} стр.</span>`:''}
                <span style="font-size:10px;color:var(--muted)">${ch.lang||''}</span>
            </div>`).join('')}`;
    }catch(e){wrap.innerHTML='<div style="color:#ff5050;font-size:13px">❌ Ошибка: '+escapeHtml(e.message)+'</div>';}
}

function mdexToggleChapter(id,num){
    const cb=document.getElementById('mdex-cb-'+id);
    const item=document.getElementById('mdex-ch-'+id);
    if(mdexSelectedChapters.has(id)){mdexSelectedChapters.delete(id);if(cb)cb.checked=false;if(item)item.classList.remove('selected');}
    else{mdexSelectedChapters.add(id);if(cb)cb.checked=true;if(item)item.classList.add('selected');}
    document.getElementById('mdex-check-all').checked=mdexSelectedChapters.size===mdexAllChapters.length;
}

function mdexToggleAll(checked){
    mdexAllChapters.forEach(ch=>{
        const cb=document.getElementById('mdex-cb-'+ch.id);
        const item=document.getElementById('mdex-ch-'+ch.id);
        if(checked){mdexSelectedChapters.add(ch.id);if(cb)cb.checked=true;if(item)item.classList.add('selected');}
        else{mdexSelectedChapters.delete(ch.id);if(cb)cb.checked=false;if(item)item.classList.remove('selected');}
    });
}

function mdexImportWrap(){
    if(!mdexSelectedChapters.size){showToast('❌ Выбери хотя бы одну главу!');return;}
    document.getElementById('mdex-import-wrap').style.display='block';
    const selected=mdexAllChapters.filter(ch=>mdexSelectedChapters.has(ch.id));
    document.getElementById('mdex-selected-chapters').innerHTML=`Выбрано: <strong>${selected.length}</strong> гл. (${selected.map(c=>'Гл.'+c.chapter).join(', ')})`;
    document.getElementById('mdex-import-wrap').scrollIntoView({behavior:'smooth',block:'nearest'});
}

async function mdexSearchExisting(){
    const q=document.getElementById('mdex-exist-search').value.trim();
    try{
        const res=await fetch(`/api/admin/manga-list?q=${encodeURIComponent(q)}&page=0&tg_user_id=`+getTgUser());
        const data=await res.json();
        const el=document.getElementById('mdex-exist-results');
        if(!data.items?.length){el.innerHTML='<div style="color:var(--muted);font-size:12px;padding:5px 0">Ничего</div>';return;}
        el.innerHTML=data.items.map(m=>`<div class="mdex-manga-item" onclick="selectExistingManga(${m.id},'${escapeHtml(m.title).replace(/'/g,"\\'")}')"><div style="flex:1;min-width:0"><div class="mdex-manga-title">${escapeHtml(m.title)}</div><div class="mdex-manga-meta">ID:${m.id}</div></div></div>`).join('');
    }catch(e){}
}
document.getElementById('mdex-exist-search').addEventListener('keydown',e=>{if(e.key==='Enter')mdexSearchExisting();});

function selectExistingManga(id,title){
    mdexExistingMangaId=id;
    document.getElementById('mdex-exist-results').innerHTML=`<div style="background:rgba(124,92,255,0.08);border:1px solid rgba(124,92,255,0.25);border-radius:8px;padding:7px 10px;font-size:12px;font-weight:600;color:var(--accent)">✅ ${escapeHtml(title)} (ID: ${id})</div>`;
    showToast('✅ Выбрана: '+title);
}

async function mdexImportSelected(){
    if(!mdexSelectedManga){showToast('❌ Нет манги!');return;}
    if(!mdexSelectedChapters.size){showToast('❌ Выбери главы!');return;}
    if(mdexAction==='add_to_existing'&&!mdexExistingMangaId){showToast('❌ Выбери существующую мангу!');return;}

    const btn=document.getElementById('mdex-import-btn');
    btn.disabled=true;btn.classList.add('loading');
    const pb=document.getElementById('mdex-progress');const pf=document.getElementById('mdex-progress-fill');
    pb.classList.add('active');pf.style.width='2%';
    const rb=document.getElementById('mdex-result');rb.className='result-banner';rb.innerHTML='';

    const selected=mdexAllChapters.filter(ch=>mdexSelectedChapters.has(ch.id));
    let success=0,fail=0,lastMangaId=null;

    for(let i=0;i<selected.length;i++){
        const ch=selected[i];
        pf.style.width=(2+Math.round((i/selected.length)*95))+'%';
        const t=btn.querySelector('.btn-text');
        if(t)t.textContent=`⏳ Гл. ${ch.chapter} (${i+1}/${selected.length})...`;

        const isFirst=(i===0);
        const body={
            action: isFirst&&mdexAction==='new_manga'?'new_manga':'add_to_existing',
            mdex_manga_id: mdexSelectedManga.id,
            mdex_chapter_id: ch.id,
            chapter_num: parseFloat(ch.chapter)||1,
            manga_title: mdexSelectedManga.title,
            cover_url: isFirst?mdexSelectedManga.coverUrl:'',
            existing_manga_id: mdexAction==='add_to_existing'?mdexExistingMangaId:(lastMangaId||0),
            lang: mdexLang||'ru',
            tg_user_id: getTgUser()
        };

        try{
            const res=await fetch('/api/mdex/import',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
            const data=await res.json();
            if(data.success){success++;if(data.manga_id&&!lastMangaId)lastMangaId=data.manga_id;if(body.action==='new_manga'&&data.manga_id){body.action='add_to_existing';}}
            else{fail++;rb.className='result-banner error open';rb.innerHTML+='❌ Гл.'+ch.chapter+': '+(data.error||'Ошибка')+'<br>';}
        }catch(e){fail++;rb.className='result-banner error open';rb.innerHTML+='❌ Гл.'+ch.chapter+': '+escapeHtml(e.message)+'<br>';}
        if(i<selected.length-1)await new Promise(r=>setTimeout(r,300));
    }

    pf.style.width='100%';
    const summary=`✅ Готово! Загружено: <strong>${success}</strong> гл.${fail?` ❌ Ошибок: ${fail}`:''}${lastMangaId?`<br><a href="/read/${lastMangaId}" target="_blank">🌐 Открыть мангу →</a>`:''}`;
    if(success>0){rb.className='result-banner success open';rb.innerHTML=summary+rb.innerHTML.replace('result-banner error open','');}
    if(success>0)setTimeout(()=>{load(true);},2000);

    const t=btn.querySelector('.btn-text');if(t)t.textContent='🚀 Импортировать выбранные главы';
